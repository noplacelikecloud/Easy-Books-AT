...
export default function CashFlowTrendWidget() {
  ...
  const chartData = {
    ...
    datasets: [
      {
        type: "line" as const, label: t('dashboard.net', 'Net'), order: 0,
      ...
      {
        type: "bar" as const, label: t('dashboard.cashIn', 'Cash In'), order: 1,
      ...
      },
      {
        ...
        type: "bar" as const, label: t('dashboard.cashOut', 'Cash Out'), order: 2,
  ...
  return (
    <TrendShell
      title={t('widget.cash_flow', 'Cash Flow')} sub={t('dashboard.cashFlowSub', 'Monthly cash in vs out with net movement')}
