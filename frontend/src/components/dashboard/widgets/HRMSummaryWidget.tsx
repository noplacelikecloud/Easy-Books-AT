  ...
  return (
    <div className="h-full flex flex-col bg-white border border-[var(--border)] rounded-xl p-4 shadow-sm">
      <div className="flex items-center justify-between mb-3">
        <p className="text-[11px] font-bold uppercase tracking-[0.12em] text-[var(--text-primary)]/55">{t('widget.hrm_summary', 'HRM & Payroll')}</p>
        <Link href="/hrm" className="text-[10px] text-[var(--primary)] hover:underline">{t('common.overview', 'Overview')} →</Link>
      </div>
      {error ? (
        ...
        <div className="flex-1 grid grid-cols-2 gap-x-3 gap-y-4 items-center">
          <Figure label={t('dashboard.activeEmployees', 'Active Employees')} value={String(data.active_employees)} />
          <Figure label={t('dashboard.lastPayrollNet', 'Last Payroll Net')} value={fmt(data.last_payroll_net)} />
          <Figure
            label={t('dashboard.pendingRuns', 'Pending Runs')}
            value={String(data.pending_runs)}
            tone={data.pending_runs > 0 ? "warning" : undefined}
          />
          <Figure
            label={t('dashboard.attendanceMtd', 'Attendance MTD')}
