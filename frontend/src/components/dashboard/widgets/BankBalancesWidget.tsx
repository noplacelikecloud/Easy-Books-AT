...
export default function BankBalancesWidget() {
  ...
  return (
    <div className="h-full flex flex-col bg-white border border-[var(--border)] rounded-xl p-4 shadow-sm">
      <p className="text-[11px] font-bold uppercase tracking-[0.12em] text-[var(--text-primary)]/55 mb-2">{t('widget.bank_balances', 'Bank Balances')}</p>
      {error ? (
        <div className="text-sm text-red-600">{t('common.failedToLoad', 'Failed to load.')}</div>
      ) : !rows ? (
        <div className="shimmer h-20 rounded-lg" />
      ) : rows.length === 0 ? (
        <div className="text-sm text-[var(--text-primary)]/40">{t('dashboard.noBankAccounts', 'No bank accounts.')}</div>
      ) : (
        <>
          <div className="flex-1 min-h-0 overflow-y-auto -mx-1 px-1">
            {rows.map(r => (
              <div key={r.id} className="flex items-center justify-between gap-2 py-1.5 border-b border-[var(--border)] last:border-0 text-sm">
                <span className="truncate text-[var(--text-primary)]/80">{r.name}</span>
                <span className="font-medium tabular-nums whitespace-nowrap">{fmt(Number(r.balance))}</span>
              </div>
            ))}
          </div>
          <div className="flex items-center justify-between pt-2 mt-1 border-t-2 border-[var(--primary)]/30 text-sm font-bold">
