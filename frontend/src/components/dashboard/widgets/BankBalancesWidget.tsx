...
export default function BankBalancesWidget() {
  ...
  return (
    <div className="h-full flex flex-col bg-white border border-[var(--border)] rounded-xl p-4 shadow-sm">
      <p className="text-[11px] font-bold uppercase tracking-[0.12em] text-[var(--text-primary)]/55 mb-2">Bank Balances</p>
      {error ? (
        <div className="text-sm text-red-600">Failed to load.</div>
      ) : !rows ? (
        <div className="shimmer h-20 rounded-lg" />
      ) : rows.length === 0 ? (
        <div className="text-sm text-[var(--text-primary)]/40">No bank accounts.</div>
      ) : (
        <>
