...
export default function DashboardGrid({ layout, ctx, editing, onExitEditing }: {
  ...
  return (
    <div className="space-y-3">
      ...
      >
        {layouts.lg.map(i => (
          <div key={i.id} className={`overflow-hidden ${editing ? "ring-2 ring-dashed ring-[var(--primary)]/40 rounded-xl relative" : ""}`}>
            {editing && (
              <button
                onClick={() => removeWidget(i.id)}
                className="no-drag absolute top-1 right-1 z-10 bg-white/90 border border-[var(--border)] rounded-full p-1 text-[var(--text-primary)]/50 hover:text-red-600"
