...
export default function LineItemsTable({ lines, onChange, products = [], taxCodes = [], showTax = false, readOnly = false, showStockHint = false, warnOversell = false, customerId = null, priceKind = 'sale', hideDiscount = false }: Props) {
  ...
  const fieldCls = "w-full bg-[var(--bg-page)] border border-[var(--border)] rounded-lg px-2 py-2 text-sm outline-none focus:ring-2 focus:ring-[var(--primary)]"
  ...
  return (
    <div className="border border-[var(--border)] rounded-xl overflow-hidden">
      ...
      {/* Phone / narrow tablet — stacked cards for realistic data entry */}
          ...
          <div key={idx} className="border border-[var(--border)] rounded-xl p-3 bg-white space-y-3">
            ...
            {hasProducts && (
              <div>
                ...
                {line.ssp != null && line.ssp > 0 && (
                  <p className="text-[10px] text-[var(--text-muted)] mt-1">SSP {fmt(line.ssp)}</p>
                )}
              </div>
            )}
            <div>
