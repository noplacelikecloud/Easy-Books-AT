...
import { calculateSizing } from "@/lib/weavingYarnCalc"
...
function SizingCalculatorInner() {
  ...
  const result = useMemo(() => calculateSizing({
    unsized_kg: num("unsized_kg"),
    pickup_pct: num("pickup_pct"),
    stretch_pct: num("stretch_pct"),
    visible_waste_pct: num("visible_waste_pct"),
    invisible_waste_pct: num("invisible_waste_pct"),
  ...
  async function doAssign(reason?: string) {
    ...
    try {
      await apiFetch("/api/weaving/calculators/sizing/assign", {
        method: "POST",
  ...
  const fields: { key: keyof typeof form; label: string }[] = [
    { key: "unsized_kg", label: "Unsized warp (kg)" },
    { key: "pickup_pct", label: "Size pickup %" },
    { key: "stretch_pct", label: "Stretch %" },
  ...
  return (
    <div className="p-4 max-w-3xl mx-auto space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <h1 className="text-xl font-semibold text-[var(--text-primary)]">Sizing Calculator</h1>
          <p className="text-sm text-[var(--text-muted)]">Unsized → sized yarn with pickup, stretch, and wastes</p>
        </div>
        <Link href="/weaving/calculators/weaving" className="text-sm text-[var(--primary)]">Weaving calculator →</Link>
      </div>
      ...
      <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-card)] p-4 space-y-2 text-sm">
        ...
        <div>Unsized: <WeightTripleDisplay triple={result.unsized} /></div>
        <div>Net (before waste): <WeightTripleDisplay triple={result.net_before_waste} /></div>
        <div className="font-medium">Gross sized: {formatWeightTriple(result.gross)}</div>
        <div className="text-[var(--text-muted)] text-xs">Waste factor {result.waste_factor.toFixed(3)}</div>
      </div>
