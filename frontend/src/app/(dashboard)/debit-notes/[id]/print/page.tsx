...
export default function DebitNotePrintPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!dn)   return <p className="p-4 text-[#1a1814]/60 text-sm">Loading…</p>
        ...
        <article className="text-[#1a1814]">
          ...
          <div className="grid grid-cols-2 gap-6 mb-6 text-sm">
            ...
            <div className="text-right">
              <div className="text-[10px] font-bold uppercase tracking-widest text-[#1a1814]/55 mb-1">Original Bill</div>
              <p className="font-semibold">Bill #{dn.bill_id}</p>
            </div>
          </div>

          {dn.description && (
            <p className="mb-4 text-sm text-[#1a1814]/75">{dn.description}</p>
          )}
