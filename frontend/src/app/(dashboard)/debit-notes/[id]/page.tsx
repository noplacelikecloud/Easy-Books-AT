...
export default function DebitNoteDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!dn) return <div className="text-center py-20 text-[#1a1814]/50">Debit note not found.</div>
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <nav className="flex items-center gap-1.5 text-xs text-[#1a1814]/50 print:hidden">
        ...
        <Link href="/debit-notes" className="hover:text-[#b8943f]">Debit Notes</Link>
        <ChevronRight className="w-3 h-3" />
        <span className="text-[#1a1814]/80 font-medium">{dn.number}</span>
      </nav>

      <div className="flex items-start justify-between gap-4">
        <div className="flex items-start gap-3">
          <div className="w-10 h-10 rounded-xl bg-[#1a1814] flex items-center justify-center flex-shrink-0">
            <Undo2 className="w-5 h-5 text-[#ffd966]" />
          </div>
          <div>
            <h1 className="text-2xl font-serif text-[#1a1814]">{dn.number}</h1>
            <p className="text-xs text-[#1a1814]/50 uppercase tracking-wide mt-0.5">Purchase Return · {dn.status}</p>
          </div>
        </div>
        <Link href={`/debit-notes/${id}/print`} className="p-3 bg-white border border-[#1a1814]/10 rounded-xl hover:bg-[#f6f3ee] text-[#1a1814]/60 print:hidden" title="Print">
      ...
      <div className="bg-white border border-[#ede9e2] rounded-xl p-5 grid grid-cols-2 gap-4 text-sm">
        ...
        <div>
          <p className="text-[10px] uppercase tracking-widest text-[#1a1814]/50">Original Bill</p>
        ...
        <div>
          ...
          <p className="mt-0.5 text-[#1a1814]/70">{dn.issue_date}</p>
        </div>
        <div>
      ...
      <div className="bg-white rounded-2xl border border-[#1a1814]/5 overflow-hidden">
        ...
        <div className="px-4 py-3 border-t border-[#ede9e2] text-sm space-y-1">
          <div className="flex justify-between"><span className="text-[#1a1814]/60">Subtotal</span><span className="font-mono">{fmt(dn.subtotal)}</span></div>
          {dn.gst_amount > 0 && <div className="flex justify-between"><span className="text-[#1a1814]/60">GST reversed</span><span className="font-mono">{fmt(dn.gst_amount)}</span></div>}
          <div className="flex justify-between font-bold border-t border-[#ede9e2] pt-1"><span>Total Return</span><span className="font-mono">{fmt(dn.total)}</span></div>
        </div>
      </div>
      <p className="text-xs text-[#1a1814]/40 italic">GL: Dr Accounts Payable / Cr Inventory (at original cost) + Cr GST Input.</p>
    </div>
  )
}
