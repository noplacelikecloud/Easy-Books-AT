...
interface Ledger {
  customer: { id: number; name: string; email: string | null; phone: string | null; opening_balance: string }
  period: { start: string | null; end: string | null }
  opening_balance: string
  entries: LedgerEntry[]
  closing_balance: string
  totals: { debit: string; credit: string; qty_out: string | null }
}
...
export default function CustomerLedgerPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    apiFetch<Ledger>(`/api/customers/${id}/ledger?start=${start}&end=${end}`)
      .then(setData)
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-4">
      <PrintHeader title={`Customer Ledger — ${c.name}`} subtitle={`Period ${start} → ${end}`} orientation="landscape" />
      ...
      <header className="bg-white border border-[var(--border)] rounded-xl p-5 flex items-start gap-3 print:hidden">
        <Users className="w-7 h-7 text-[var(--primary)] shrink-0 mt-1" />
        <div className="min-w-0">
          <h1 className="text-2xl font-bold text-[var(--text-primary)] truncate">{c.name}</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 print:hidden">
        ...
        <div className="bg-white border border-[var(--border)] rounded-xl p-4 grid grid-cols-3 gap-3 text-center">
          ...
          />
          {data.totals.qty_out && (
            <Stat label="Qty sold" value={data.totals.qty_out} />
          )}
        </div>
      </div>
              ...
              return (
                <tr key={i}>
                  ...
                  <td className="px-3 py-2 font-mono text-xs whitespace-nowrap">
                    {href ? (
                      <Link href={href} className="text-[var(--primary)] hover:underline">{e.doc_number}</Link>
                    ) : e.doc_number}
                  </td>
