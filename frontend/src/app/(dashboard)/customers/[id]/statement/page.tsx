  ...
  return (
    <div className="max-w-4xl mx-auto space-y-4">
      ...
      <div className="flex items-center justify-between gap-2 print:hidden">
        <Link href={`/customers/${id}/ledger`} className="inline-flex items-center gap-1.5 px-3 py-2 text-sm text-[var(--text-primary)]/65 hover:text-[var(--primary)]">
      ...
      {/* Statement document */}
      <div className="bg-white border border-[var(--border)] rounded-xl overflow-hidden print:border-none print:rounded-none print:shadow-none">
        {/* Customer info header */}
        <div className="p-6 border-b border-[var(--border)]">
          <div className="flex justify-between items-start">
            <div>
              <h2 className="text-xl font-bold">{c.name}</h2>
              {c.email && <p className="text-sm text-[var(--text-muted)] mt-0.5">{c.email}</p>}
              {c.phone && <p className="text-sm text-[var(--text-muted)]">{c.phone}</p>}
              {c.address && <p className="text-sm text-[var(--text-muted)]">{c.address}</p>}
            </div>
            <div className="text-right">
              <p className="text-xs text-[var(--text-muted)] uppercase tracking-widest font-bold">Account Statement</p>
        ...
        <div className="grid grid-cols-3 divide-x divide-[var(--border)] border-b border-[var(--border)] bg-[var(--bg-page)]">
          ...
          <div className="px-6 py-4 text-center">
            <p className="text-xs text-[var(--text-muted)] uppercase tracking-widest font-bold mb-1">Invoices Issued</p>
        ...
        {/* Invoices */}
        <div className="p-6">
          <h3 className="text-xs font-bold uppercase tracking-widest text-[var(--text-muted)] mb-3">Invoices</h3>
          {data.invoices.length === 0 ? (
            <p className="text-sm text-[var(--text-muted)] italic">No invoices in this period.</p>
          ) : (
            <div className="table-freeze">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[var(--border)]">
                  <th className="py-2 text-left text-[10px] font-bold uppercase tracking-widest text-[var(--text-muted)]">Invoice #</th>
              ...
              <tbody className="divide-y divide-[var(--border)]">
                {data.invoices.map(inv => (
                  <tr key={inv.id}>
                    <td className="py-2 font-mono text-[var(--primary)] text-xs">{inv.number}</td>
                    <td className="py-2 text-[var(--text-muted)]">{fmtDate(inv.date)}</td>
                    <td className="py-2 text-[var(--text-muted)]">{inv.due_date}</td>
        ...
        <div className="px-6 pb-6">
          <h3 className="text-xs font-bold uppercase tracking-widest text-[var(--text-muted)] mb-3">Payments Received</h3>
        ...
        <div className="px-6 py-4 bg-[var(--bg-page)] border-t border-[var(--border)] flex justify-between items-center">
          ...
          <div className="text-right">
            <p className="text-xs text-[var(--text-muted)] uppercase tracking-widest font-bold">Balance Due</p>
