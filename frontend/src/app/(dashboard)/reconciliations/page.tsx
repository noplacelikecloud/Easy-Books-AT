  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 print:hidden">
        <div>
          <h1 className="text-xl sm:text-3xl font-bold">Reconciliations</h1>
          <p className="text-sm text-[var(--text-muted)] mt-1">Bank reconciliation and cash verification</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={openNew} className="flex items-center gap-2 px-4 py-2 bg-[var(--primary)] text-white rounded-lg hover:bg-[var(--primary-dark)]">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-white rounded-lg border border-[var(--border)] p-6">
          <p className="text-xs text-[var(--text-muted)] uppercase tracking-widest font-bold">Completed</p>
          <p className="text-xl sm:text-3xl font-bold text-green-600 mt-2">{recs.filter(r => r.status === 'closed').length}</p>
        </div>
        <div className="bg-white rounded-lg border border-[var(--border)] p-6">
          <p className="text-xs text-[var(--text-muted)] uppercase tracking-widest font-bold">Open</p>
      ...
      <div className="space-y-3">
        {loading ? (
          ...
          <div className="text-center py-12 text-[var(--text-muted)]">No reconciliations yet. Create one to verify your cash balance.</div>
        ) : recs.map(rec => (
          <div key={rec.id} className="bg-white rounded-lg border border-[var(--border)] p-6">
            <div className="flex items-start justify-between">
              <div>
                <h3 className="font-semibold">{rec.bank_account_name}</h3>
                <p className="text-xs text-[var(--text-muted)] mt-1">{rec.period_start} — {rec.period_end}</p>
              </div>
              <div className="flex items-center gap-3">
                ...
                {rec.status === 'open' && (
                  <button onClick={() => openDetail(rec)} className="text-[var(--primary)] font-bold text-sm hover:underline">Open</button>
                )}
              </div>
            </div>
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 p-8">
            <h2 className="text-2xl font-bold text-[var(--text-primary)] mb-6">New Reconciliation</h2>
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-bold uppercase tracking-widest text-[var(--text-primary)]/75 mb-1">Bank Account</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold uppercase tracking-widest text-[var(--text-primary)]/75 mb-1">Period Start</label>
                  <input type="date" value={form.period_start} onChange={e => setForm(p => ({ ...p, period_start: e.target.value }))}
                    className="w-full ui-field bg-[var(--bg-page)] rounded-xl outline-none focus:ring-2 focus:ring-[var(--primary)]" />
                </div>
                <div>
                  <label className="block text-xs font-bold uppercase tracking-widest text-[var(--text-primary)]/75 mb-1">Period End</label>
                  <input type="date" value={form.period_end} onChange={e => setForm(p => ({ ...p, period_end: e.target.value }))}
                    className="w-full ui-field bg-[var(--bg-page)] rounded-xl outline-none focus:ring-2 focus:ring-[var(--primary)]" />
                </div>
              </div>
              <div>
                <label className="block text-xs font-bold uppercase tracking-widest text-[var(--text-primary)]/75 mb-1">Bank Statement Balance</label>
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button onClick={handleCreate} disabled={saving} className="px-6 py-3 bg-[var(--text-primary)] text-white rounded-xl font-bold hover:bg-[var(--primary)] hover:text-black transition-all disabled:opacity-50">
      ...
      {detail && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl mx-4 p-8 overflow-y-auto max-h-[90vh]">
            <h2 className="text-2xl font-bold text-[var(--text-primary)] mb-2">Reconcile: {detail.bank_account_name}</h2>
            <p className="text-sm text-[var(--text-muted)] mb-6">{detail.period_start} — {detail.period_end}</p>

            <div className="grid grid-cols-3 gap-4 mb-6">
              <div className="bg-[var(--bg-page)] rounded-xl p-4 text-center">
                <p className="text-xs text-[var(--text-muted)] uppercase tracking-widest mb-1">Statement</p>
                <p className="text-lg font-bold font-mono">{fmt(detail.statement_balance)}</p>
              </div>
              <div className="bg-[var(--bg-page)] rounded-xl p-4 text-center">
                <p className="text-xs text-[var(--text-muted)] uppercase tracking-widest mb-1">Matched GL</p>
              ...
              <div className={`rounded-xl p-4 text-center ${Math.abs(difference) < 0.01 ? 'bg-green-50' : 'bg-red-50'}`}>
                <p className="text-xs text-[var(--text-muted)] uppercase tracking-widest mb-1">Difference</p>
                ...
                {adjOpen && (
                  <div className="mt-3 pt-3 border-t border-amber-200 space-y-3">
                    ...
                    <button onClick={handlePostAdj} disabled={adjSaving}
                      className="w-full py-2 bg-[var(--text-primary)] text-white rounded-xl font-bold text-sm hover:bg-[var(--primary)] hover:text-black transition-all disabled:opacity-50">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-[var(--bg-page)]">
                    ...
                    <th className="ui-th text-left text-xs font-bold uppercase tracking-widest text-[var(--text-muted)]">{t('col.description', 'Description')}</th>
                    <th className="ui-th text-right text-xs font-bold uppercase tracking-widest text-[var(--text-muted)]">{t('col.debit', 'Debit')}</th>
                    <th className="ui-th text-right text-xs font-bold uppercase tracking-widest text-[var(--text-muted)]">{t('col.credit', 'Credit')}</th>
                    <th className="ui-th text-center text-xs font-bold uppercase tracking-widest text-[var(--text-muted)]">Clear</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-[var(--border)]">
                  {detail.lines.length === 0 ? (
                    <tr><td colSpan={5} className="ui-td text-center text-[var(--text-muted)]">No GL entries for this period/account.</td></tr>
                  ) : detail.lines.map(ln => (
                    <tr key={ln.id} className={ln.is_matched ? 'bg-green-50/60' : ''}>
                      <td className="ui-td text-[var(--text-muted)]">{fmtDate(ln.date)}</td>
                      ...
                      <td className="ui-td text-center">
                        <button onClick={() => toggleLine(detail.id, ln.id, !ln.is_matched)}>
                          {ln.is_matched
                            ...
                            : <XCircle className="w-5 h-5 text-[var(--border)] mx-auto hover:text-[var(--primary)]" />}
                        </button>
                      </td>
                    </tr>
                  ))}
            ...
            <div className="flex justify-end gap-3">
              <button onClick={() => setDetail(null)} className="px-6 py-3 border border-[var(--text-primary)]/10 rounded-xl font-bold hover:bg-[var(--bg-page)]">Close</button>
              <button
