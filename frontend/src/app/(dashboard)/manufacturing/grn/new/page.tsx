  ...
  return (
    <div className="max-w-3xl space-y-6">
      <div>
        ...
        <h1 className="text-2xl font-bold text-[var(--text-primary)] mt-2">New Goods Receipt Note</h1>
        <p className="text-sm text-[var(--text-primary)]/60 mt-0.5">Record customer-supplied material received into your godown.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="bg-white border border-[var(--border)] rounded-xl p-5 space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-semibold text-[var(--text-primary)]/70 mb-1.5 uppercase tracking-wide">Received Date</label>
              <input
                type="date"
                value={receivedDate}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold text-[var(--text-primary)]/70 mb-1.5 uppercase tracking-wide">Godown / Location</label>
              <select
                value={locationId}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[var(--border)] bg-[#faf8f4]">
                  <th className="text-left px-4 py-2.5 font-semibold text-[var(--text-primary)]/70">{t('col.product', 'Product')}</th>
                  <th className="text-left px-3 py-2.5 font-semibold text-[var(--text-primary)]/70 w-24">Qty</th>
                  <th className="text-left px-3 py-2.5 font-semibold text-[var(--text-primary)]/70 w-28">Lot No.</th>
                  <th className="text-left px-3 py-2.5 font-semibold text-[var(--text-primary)]/70 w-28">Declared Value</th>
                  <th className="text-left px-3 py-2.5 font-semibold text-[var(--text-primary)]/70">{t('col.notes', 'Notes')}</th>
                  ...
                  <tr key={i} className="border-b border-[var(--border)] last:border-0">
                    ...
                    <td className="px-3 py-2">
                      <input
                        ...
                        placeholder="optional"
                        className="w-full border border-[#d4cfc7] rounded px-2 py-1.5 text-xs font-mono focus:outline-none focus:border-[var(--primary)]"
                      />
                    </td>
                    <td className="px-3 py-2">
                      <input
              ...
              {totalDeclared > 0 && (
                <tfoot>
                  <tr className="border-t border-[var(--border)] bg-[#faf8f4]">
                    <td className="px-4 py-2 text-xs text-[var(--text-primary)]/60" colSpan={3}>
                      Total declared value (memo JE will be posted)
                    </td>
                    <td className="px-3 py-2 text-right text-xs font-bold tabular-nums text-[var(--text-primary)]">
            ...
            {lines.map((line, i) => (
              <div key={i} className="p-4 space-y-3">
                <div className="flex justify-between items-center">
                  <span className="text-xs font-semibold text-[var(--text-primary)]/60">Line {i + 1}</span>
                  {lines.length > 1 && (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    <label className="text-xs text-[var(--text-primary)]/60 mb-1 block">Qty</label>
                    <input
                  ...
                  <div>
                    <label className="text-xs text-[var(--text-primary)]/60 mb-1 block">Declared Value</label>
                    <input
