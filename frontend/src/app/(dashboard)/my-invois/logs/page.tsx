...
export default function MyInvoisLogsPage() {
  useBreadcrumb("MyInvois Submission Logs")
  const [logs, setLogs] = useState<MyInvoisLog[]>([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    apiFetch<MyInvoisLog[]>("/api/my-invois/logs?limit=100")
      .then(setLogs)
      .catch(() => {})
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h1 className="text-xl sm:text-3xl font-bold">MyInvois Submission Logs</h1>
          <p className="text-sm text-[var(--text-muted)] mt-1">
            Audit trail of LHDN document submits (sandbox or production)
          </p>
        </div>
        <Link href="/my-invois" className="text-sm font-semibold text-[var(--primary)] hover:underline">
          ← Dashboard
        </Link>
      </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {!loading && logs.length === 0 && (
                <tr>
                  <td colSpan={7} className="px-3 py-8 text-center text-[var(--text-muted)]">
                    No submissions yet. Open an invoice and use &quot;Submit to MyInvois&quot;.
                  </td>
                </tr>
              )}
              {logs.map((log) => (
                <tr key={log.id} className="border-b border-[var(--border-light)]">
                  ...
                  <td className="px-3 py-2 font-mono text-xs">{log.uuid || "—"}</td>
