    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="bg-white border border-[#ede9e2] rounded-xl px-6 py-12 text-center">
          <Layers className="w-10 h-10 text-[#b8943f]/40 mx-auto mb-3" />
          <p className="text-sm font-medium text-[#1a1814]">No analytic accounts yet</p>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-[#ede9e2] bg-[#faf8f4]">
              <th className="text-left px-4 py-3 font-semibold text-[#1a1814]/70 w-28">Code</th>
              <th className="text-left px-4 py-3 font-semibold text-[#1a1814]/70">Name</th>
              <th className="text-left px-4 py-3 font-semibold text-[#1a1814]/70">Type</th>
              <th className="px-4 py-3 w-24" />
            </tr>
          </thead>
          <tbody>
            {items.map(aa => (
              <tr key={aa.id} className="border-b border-[#ede9e2] last:border-0 hover:bg-[#faf8f4]">
                <td className="px-4 py-3 font-mono text-xs text-[#1a1814]/80">{aa.code}</td>
                <td className="px-4 py-3 text-[#1a1814]">{aa.name}</td>
                ...
                <td className="px-4 py-3 text-right">
                  <div className="flex items-center justify-end gap-2">
                    ...
                    <button
                      onClick={() => onToggle(aa)}
                      className={`transition-colors ${aa.is_active ? "text-emerald-500 hover:text-red-400" : "text-[#1a1814]/30 hover:text-emerald-500"}`}
