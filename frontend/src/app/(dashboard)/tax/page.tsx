    ...
    <div className="space-y-6 max-w-5xl">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 print:hidden">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={exportCsv} disabled={!data} className="p-3 bg-white border border-[var(--border)] rounded-xl hover:bg-[var(--bg-page)] transition-colors text-[var(--text-primary)]/60 disabled:opacity-40 print:hidden" title="Export CSV">
            <Download className="w-5 h-5" />
          </button>
          <button onClick={() => window.print()} className="p-3 bg-white border border-[var(--border)] rounded-xl hover:bg-[var(--bg-page)] transition-colors text-[var(--text-primary)]/60 print:hidden" title="Print">
      ...
      {!data ? (
        ...
        <>
          ...
          {/* Income Tax Section */}
          <div className="bg-white rounded-xl border border-[var(--border)] p-8">
            <h2 className="text-lg font-semibold mb-6 pb-3 border-b border-[var(--border)]">Income Tax Estimate</h2>
            <div className="space-y-3 mb-6">
              ...
              <div className="flex justify-between py-3 border-t-2 border-[var(--text-primary)] font-semibold text-lg">
                <span>Taxable Income</span>
                <span className="font-mono text-[var(--primary)]">{fmt(data.income_tax.taxable_income)}</span>
              </div>
            </div>

            <div className="bg-[var(--bg-page)] rounded-xl p-6 space-y-3">
              <div className="flex justify-between text-sm">
                <span className="text-[var(--text-muted)]">Basis</span>
                <span className="text-[var(--text-primary)]">{data.income_tax.tax_basis}</span>
              </div>
              <div className="flex justify-between pt-3 border-t border-[var(--border)] font-semibold text-lg">
