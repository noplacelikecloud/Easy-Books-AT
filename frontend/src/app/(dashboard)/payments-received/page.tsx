  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 print:hidden">
        <div>
          <h1 className="text-xl sm:text-3xl font-bold">Payments Received</h1>
          <p className="text-sm text-[var(--text-muted)] mt-1">Record customer payments and track cash receipts</p>
        </div>
      ...
      <div className="bg-white rounded-lg border border-[var(--border)] p-6">
        <p className="text-xs text-[var(--text-muted)] uppercase tracking-widest font-bold">Total Received (this page)</p>
          ...
          <table className="w-full text-sm min-w-[520px]">
            <thead className="bg-[var(--bg-page)] border-b border-[var(--border)]">
              <tr>
                ...
                <th className="ui-th text-left text-xs font-bold uppercase tracking-widest text-[var(--text-muted)]">{t('col.customer', 'Customer')}</th>
            ...
            <tbody className="divide-y divide-[var(--border)]">
              {loading ? (
                ...
                <tr><td colSpan={6} className="px-6 py-8 text-center text-[var(--text-muted)]">No payments recorded.</td></tr>
              ) : filtered.map(p => (
                <tr key={p.id} className="hover:bg-[var(--bg-page)]/50">
                  ...
                  <td className="ui-td font-mono text-sm text-[var(--text-muted)]">
                    {p.invoice_id
                      ? <DocLink type="invoice" id={p.invoice_id} label={p.reference ?? `INV #${p.invoice_id}`} className="text-[var(--text-muted)]" />
                      : (p.reference ?? '—')}
                  </td>
