...
export default function CreditNotePrintPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!cn)   return <p className="p-4 text-[var(--text-primary)]/60 text-sm">Loading…</p>
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 pb-10">
        ...
        <article className="text-[var(--text-primary)]">
          <header className="mb-6 print:hidden border-b border-[var(--border)] pb-4">
            <h1 className="text-lg sm:text-2xl font-bold">Credit Note {cn.number}</h1>
            <p className="text-sm text-[var(--text-primary)]/60">Issued {fmtDate(cn.issue_date)} · {cn.status}</p>
          </header>

          <div className="grid grid-cols-2 gap-6 mb-6 text-sm">
            <div>
              <div className="text-[10px] font-bold uppercase tracking-widest text-[var(--text-primary)]/55 mb-1">{t('col.customer', 'Customer')}</div>
              <p className="font-semibold">{cn.customer_name ?? "—"}</p>
            </div>
            <div className="text-right">
              {cn.invoice_id && (
                <div>
                  <div className="text-[10px] font-bold uppercase tracking-widest text-[var(--text-primary)]/55 mb-1">Original Invoice</div>
          ...
          {cn.description && (
            <p className="mb-4 text-sm text-[var(--text-primary)]/75">{cn.description}</p>
          )}
