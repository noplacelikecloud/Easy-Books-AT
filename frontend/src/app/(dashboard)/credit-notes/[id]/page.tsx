...
export default function CreditNoteDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!cn) return <div className="text-center py-20 text-[var(--text-primary)]/50">Credit note not found.</div>
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="bg-white border border-[var(--border)] rounded-xl p-5 grid grid-cols-2 gap-4 text-sm">
        <div>
          <p className="text-[10px] uppercase tracking-widest text-[var(--text-primary)]/50">{t('col.customer', 'Customer')}</p>
          <p className="mt-0.5">{cn.customer_id
            ? <DocLink type="customer" id={cn.customer_id} label={cn.customer_name ?? "Customer"} />
            : (cn.customer_name ?? "—")}</p>
        </div>
        <div>
          <p className="text-[10px] uppercase tracking-widest text-[var(--text-primary)]/50">Original Invoice</p>
        ...
        <div>
          ...
          <p className="mt-0.5 text-[var(--text-primary)]/70">{cn.issue_date}</p>
        </div>
        <div>
