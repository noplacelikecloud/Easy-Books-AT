...
export default function ProductStockCardPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!data)          return <p className="p-4 text-[var(--text-primary)]/60 text-sm">Loading stock card…</p>
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 print:hidden">
        <Link href={`/products/${id}`} className="inline-flex items-center gap-1.5 px-3 py-2 text-sm text-[var(--text-primary)]/65 hover:text-[var(--primary)]">
      ...
      <header className="bg-white border border-[var(--border)] rounded-xl p-5 flex items-start gap-3 print:hidden">
        <Package className="w-7 h-7 text-[var(--primary)] shrink-0 mt-1" />
        <div className="min-w-0">
          <h1 className="text-2xl font-bold text-[var(--text-primary)] truncate">{p.name}</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 print:hidden">
        ...
        <div className="bg-white border border-[var(--border)] rounded-xl p-4 grid grid-cols-4 gap-3 text-center">
        ...
        <table className="w-full text-xs min-w-[920px]">
          <thead className="bg-[var(--bg-page)]">
            <tr>
              ...
              <th className="text-left px-3 py-2 text-[10px] font-bold uppercase tracking-widest text-[var(--text-primary)]/55">Direction</th>
              <th className="text-left px-3 py-2 text-[10px] font-bold uppercase tracking-widest text-[var(--text-primary)]/55">Lot</th>
              <th className="text-left px-3 py-2 text-[10px] font-bold uppercase tracking-widest text-[var(--text-primary)]/55">Source</th>
              <th className="text-right px-3 py-2 text-[10px] font-bold uppercase tracking-widest text-[var(--text-primary)]/55 w-20">Qty in</th>
              <th className="text-right px-3 py-2 text-[10px] font-bold uppercase tracking-widest text-[var(--text-primary)]/55 w-20">Qty out</th>
              <th className="text-right px-3 py-2 text-[10px] font-bold uppercase tracking-widest text-[var(--text-primary)]/55 w-24">Unit cost</th>
              <th className="text-right px-3 py-2 text-[10px] font-bold uppercase tracking-widest text-[var(--text-primary)]/55 w-24">Run qty</th>
              <th className="text-right px-3 py-2 text-[10px] font-bold uppercase tracking-widest text-[var(--text-primary)]/55 w-28">Run value</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-[var(--border)]">
            <tr className="bg-[#faf8f4]">
              <td colSpan={7} className="px-3 py-2 text-[11px] font-bold text-[var(--text-primary)]/55">Opening</td>
            ...
            {data.entries.map((m, i) => {
              ...
              return (
                <tr key={i}>
                  <td className="px-3 py-2 text-[var(--text-primary)]/70">{fmtDate(m.date)}</td>
                  ...
                  <td className="px-3 py-2 font-mono">
                    {m.source_doc_type ? (
                      href ? (
                        <Link href={href} className="text-[var(--primary)] hover:underline">{m.source_doc_type}#{m.source_doc_id}</Link>
          ...
          <tfoot>
            <tr className="border-t-2 border-[var(--text-primary)] bg-[var(--bg-page)]">
              <td colSpan={4} className="px-3 py-2 text-[10px] font-bold uppercase tracking-widest text-[var(--text-primary)]/55">Closing</td>
