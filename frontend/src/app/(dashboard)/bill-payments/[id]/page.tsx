...
export default function BillPaymentDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!pay)          return <p className="p-4 text-[#1a1814]/60 text-sm">Loading payment…</p>
    ...
    <div className="max-w-4xl mx-auto space-y-4">
      ...
      <header className="bg-white border border-[#ede9e2] rounded-xl p-5 flex items-start gap-3">
        ...
        <div className="min-w-0">
          <h1 className="text-2xl font-serif font-semibold text-[#1a1814]">Bill Payment #{pay.id}</h1>
      ...
      <section className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div className="bg-white border border-[#ede9e2] rounded-xl p-4">
          ...
          {pay.vendor_id ? (
            <Link href={`/vendors/${pay.vendor_id}/ledger`} className="font-semibold text-[#1a1814] hover:text-[#b8943f] hover:underline">
      ...
      <section className="bg-white border border-[#ede9e2] rounded-xl overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-[#faf6ec]">
            <tr>
              <th className="text-left px-4 py-2 text-[10px] font-bold uppercase tracking-widest text-[#1a1814]/55">Bill</th>
            ...
            {pay.allocations.length === 0 ? (
              ...
              <tr key={a.id}>
                <td className="px-4 py-2">
                  {a.bill_id ? (
                    <Link href={`/bills/${a.bill_id}`} className="font-mono text-[#b8943f] hover:underline">
