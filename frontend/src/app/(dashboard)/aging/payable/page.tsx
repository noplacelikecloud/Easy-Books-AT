  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4 print:hidden">
        <div>
          <h1 className="text-xl sm:text-3xl font-bold text-[var(--text-primary)]">{t('reports.apAging', 'AP Aging')}</h1>
          <p className="text-[var(--text-primary)]/60">{t('reports.apAgingSub', 'Outstanding payables by age bucket')}</p>
        </div>
      ...
      <PrintHeader title={t('reports.apAging', 'AP Aging Report')} subtitle={`${t('reports.asOf', 'As of')} ${fmtDateJs(new Date())}`} orientation="landscape" />
          ...
          <table className="w-full text-left border-collapse min-w-[600px]">
            <thead>
              <tr className="bg-[var(--bg-page)] border-b border-[var(--text-primary)]/5">
                <th className="ui-th text-xs font-bold uppercase tracking-widest text-[var(--text-primary)]/75">{t('col.vendor', 'Vendor')}</th>
                <th className="ui-th text-xs font-bold uppercase tracking-widest text-[var(--text-primary)]/75">{t('col.billNumber', 'Bill #')}</th>
              ...
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={8} className="px-6 py-10 text-center text-[var(--text-primary)]/75">
                    {selectedBucket ? t('reports.noItemsInBucket', 'No items in this bucket.') : t('reports.noOutstandingBills', 'No outstanding payables.')}
                  </td>
                </tr>
              ) : (
