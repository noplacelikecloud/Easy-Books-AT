...
export default function Page() {
  ...
  useEffect(() => {
    apiFetch<Row[]>("/api/textile-processing/pakki-parchis").then(d => setRows(Array.isArray(d) ? d : [])).catch(() => setRows([]))
  }, [])
  return (
      ...
      <div className="table-freeze overflow-auto border border-[var(--border)] rounded-xl">
        <table className="w-full text-sm">
          <thead><tr className="text-left border-b border-[var(--border)]">
            ...
            <th className="p-2 text-right">Safi MTR</th><th className="p-2">Thans</th>
            ...
            {(rows || []).map(r => (
              <tr key={r.id} className="border-b border-[var(--border)]/60">
                <td className="p-2 whitespace-nowrap">
                  <Link href={`/processing/pakki-parchi/${r.id}`} className="text-[var(--primary)]">{r.number}</Link>
                </td>
                ...
                <td className="p-2 print:hidden">
                  <Link href={`/processing/pakki-parchi/${r.id}`} className="text-xs text-[var(--primary)]">Open / Print</Link>
                </td>
              </tr>
            ))}
            {rows && rows.length === 0 && (
