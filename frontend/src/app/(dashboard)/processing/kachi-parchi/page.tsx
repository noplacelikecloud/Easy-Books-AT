...
export default function Page() {
  ...
  function load() {
    apiFetch<any[]>("/api/textile-processing/kachi-parchis").then(d => setRows(Array.isArray(d) ? d : [])).catch(() => setRows([]))
  }
  useEffect(() => { load() }, [])
  return (
    <div className="p-4 space-y-4 max-w-6xl mx-auto">
      ...
      <div className="table-freeze overflow-auto border border-[var(--border)] rounded-xl">
        <table className="w-full text-sm">
          <thead><tr className="text-left border-b border-[var(--border)]"><th className="p-2">Number</th><th className="p-2">Date</th><th className="p-2">Lot</th><th className="p-2">Meters</th><th className="p-2">Thans</th></tr></thead>
          <tbody>
            {(rows || []).map(r => (
