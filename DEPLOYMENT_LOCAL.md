...
- **User preferences (theme, language, sidebar, dashboard layout):** stored in the browser's `localStorage` — not in the database. Keys: `eb.theme` (light/dark/system), `eb.color` (color theme), `eb.lang` (language), `eb.sidebar.pinned`, `eb.sidebar.open`. These preferences survive application updates automatically because they live in the browser, not in `~/.easy-books`. In Electron, the same Chromium `localStorage` persists across app restarts. No migration is needed when upgrading; no accounting data is affected. The `app_language` setting is additionally synced to `/api/settings` so the chosen language follows the user when they switch browsers.
