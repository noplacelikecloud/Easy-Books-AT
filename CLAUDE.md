...
| `routers/ai_chat.py` | AI Financial Assistant (#112 L1) — `POST /api/ai/chat`: Anthropic agent loop (max 6 steps, `claude-sonnet-4-6`) over 7 read-only tools that call existing report functions directly (tenant filters reused, never re-implemented). Gates: `ANTHROPIC_API_KEY` unset → 503, `ai_assistant` module not installed → 403, message > 4,000 chars → 400; history trimmed to last 20 turns; typed SDK errors → 503/429/502; failed tools return `is_error` results. Sync endpoint by design (matches app convention). |
...
**AI Chat (`components/AIChat.tsx` + `AIChatButton.tsx`):** Sparkles FAB rendered in `(dashboard)/layout.tsx`, hidden unless the `ai_assistant` module is installed (`useModules()`); opens a `createPortal` chat panel with quick prompts and session-only history; calls `POST /api/ai/chat` via `apiFetch`. Backend counterpart: `routers/ai_chat.py`.
...
SEED_ADMIN_PASSWORD=
SEED_COMPANY_NAME=
ANTHROPIC_API_KEY=         # enables the AI Financial Assistant (optional; endpoint returns 503 without it)
```
