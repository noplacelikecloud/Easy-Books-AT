...
| `routers/ai_chat.py` | AI Financial Assistant (#112 L1, multi-provider #117) — `POST /api/ai/chat` is an **async** SSE endpoint (`text/event-stream`) driving a LiteLLM agent loop (max 6 steps) over 7 read-only tools that call existing report functions directly (tenant filters reused, never re-implemented). Providers: anthropic/openai/gemini via LiteLLM, each with its own model list; API keys resolve tenant-first from the Settings KV (`ai_api_key_<provider>`) with `ANTHROPIC_API_KEY` env var as a dev/demo fallback for the anthropic provider only (see `services/ai_providers.py`). Chat sessions are server-side and per-user-private (`AiChatSession`/`AiChatMessage`, filtered by `tenant_id` + `user_id`); `GET/POST /sessions`, `PATCH/DELETE /sessions/{id}`, `GET /sessions/{id}/messages`. Streamed events: `token` (text delta), `tool_start`/`tool_end` (tool-call progress labels), `done` (`session_id`+`message_id`, persists the assistant reply), `error` (mid-stream failure — headers already sent, so provider errors can't become HTTP status codes once streaming starts). Pre-stream gates (plain HTTP): `ai_assistant` module not installed → 403, message > 4,000 chars → 400, no provider configured → 503, unknown/misconfigured model → 400, sliding-hour rate limit (`ai_rate_limit_per_hour` setting, default 20/hour, per-process in-memory) → 429. History trimmed to last 20 turns. `GET /models` returns configured providers + default model for the chat UI's dropdown; `GET /key-status` (admin-only) returns masked (`••••1234`) key tails per provider, never raw values. |
...
| `routers/purchase_reports.py` | Gate register (vehicle/challan search) + 3-way match (PO vs Σ GI vs Bill, positional line match, variance flags). Vendor Performance (delivery lead time, quotation rate trend, short-receipt-rate proxy for rejection rate). |
...
| `services/ai_providers.py` | Multi-provider LLM registry for the AI Financial Assistant (#117) — single source of truth for `PROVIDERS` (anthropic/openai/gemini: label, `settings_key`, `env_fallback`, `models`), `resolve_api_key()` (tenant Settings KV wins, env fallback for anthropic only), `configured_providers()` (the `/api/ai/models` payload — only providers with a resolvable key, registry-order fallback default), `validate_model()` (parses `provider/model` strings, raises `ValueError` on unknown/unconfigured), `mask_key()` (`••••1234` — never returns raw key material). `AI_SECRET_SETTINGS_KEYS` is consumed by `routers/settings.py`'s `GET /api/settings` to redact `ai_api_key_*` before the response leaves the server. |
| `services/` | Pure-logic modules — `posting.py` is the only GL writer; also `account_tree.py`, `deferred.py`, `depreciation.py`, `pdf.py`, `email.py`, `permissions.py`, `ai_providers.py` |
| `scripts/seed_demo.py` | Idempotent rich mock-data seeder (50+ per entity type). Exercises current features: data spans **two fiscal years**, transactions carry **voucher types** (SL/PU/CR/CP/CN/DN), the services tenant demonstrates **deferred-revenue origination with partial recognition**, and each tenant has **multiple users** (owner/accountant/clerk) with varied Audit-Log attribution. `_seed_purchase_store_chain` (manufacturing tenant only) exercises the full #137 P1/P2/P2b chain end-to-end: demands across every status, comparatives at lowest-wins/justify-required/pending-approval, POs at partial/full/short-received and billed/unbilled, a Gate Inward cancel-and-re-enter, and Gate Outward across all three source types including an approved scrap entry with real GL postings — every Purchases/Store screen and report has data on first login. |
